...
export function fileBrowserReducer(fileBrowserState: FileBrowserState, fileBrowserAction: Action) {
  return createReducer(
    ...
    on(FileBrowserActions.actionSelectFile, (state: FileBrowserState, action) => ({
      ...state,
      selectedFile: action.file
    })),
    on(FileBrowserActions.actionToggleFile, (state: FileBrowserState, action) => {
      ...
      if (state.selectedFile) {
        ...
        const oldSelectedFileIndex = folderContentCopy.findIndex(value => value.name === state.selectedFile.name);
        folderContentCopy = [
